                      ...
                      <tr>
                        ...
                        <td>
                          @php
                            ...
                            $statusText = match($registration->workflow_status ?? 'submitted') {
                              'submitted' => 'Chờ trưởng phòng',
                              'dept_review' => 'Chờ giám đốc',
                              'director_review' => 'Chờ giám đốc',
                        ...
                        <td>
                          <div class="d-flex gap-1">
                            ...
                            {{-- Department Head Approval (Trưởng phòng) - CHỈ trưởng phòng mới thấy --}}
                            @if($registration->workflow_status === 'submitted' && $registration->user_id != auth()->user()->id && auth()->user()->hasAnyRole(['Admin', 'HR']))
                            ...
                            {{-- Director Approval (Giám đốc) - CHỈ giám đốc mới thấy --}}
                            @if($registration->workflow_status === 'dept_review' && $registration->user_id != auth()->user()->id && auth()->user()->hasAnyRole(['Admin', 'CC']))
                              <button wire:click.prevent="approveDirector({{ $registration->id }})" 
                                      class="btn btn-sm btn-primary" 
                                      title="Phê duyệt (Giám đốc)">
                                <i class="ti ti-check-double"></i> Giám đốc duyệt
                              </button>
                              <button wire:click.prevent="rejectRegistration({{ $registration->id }})" 
                                      class="btn btn-sm btn-danger" 
                                      title="Từ chối">
                            ...
                            @if($registration->user_id == auth()->user()->id && $registration->workflow_status === 'submitted')
                              ...
                              <button wire:click.prevent="deleteRegistration({{ $registration->id }})" 
                                      class="btn btn-sm btn-danger" 
                                      title="Xóa đăng ký"
                            ...
                            @if(in_array($registration->workflow_status, ['dept_review', 'approved']))
                              <button wire:click.prevent="downloadVehiclePDF({{ $registration->id }})" 
                                ...
                                <i class="ti ti-download"></i> Tải PDF
                              </button>
                            @endif
                          </div>
                        </td>
